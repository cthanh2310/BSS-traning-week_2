...
function renderTable(dataDashboard) {
  let content = dataDashboard.map((value, index) => {
    return `<tr class="statistic-tr">
        ...
        <td>
          <i onClick="deleteDataRow(${index})" class="fa-solid fa-circle-xmark" style="font-size:1.8rem; color: red; cursor: pointer; margin-left: 4px">
          </i>
          <i onClick="editDataRow(${index})" class="fa-solid fa-pencil" style="font-size:1.8rem; color: blue; cursor: pointer; margin-left: 4px"></i>
        </td>
        </tr>`;
  });
